...
调用 `KiyoMessageBox.alert` 方法以打开 alert 框。
...
调用 `KiyoMessageBox.confirm` 方法以打开 confirm 框。
...
调用 `KiyoMessageBox.prompt` 方法以打开 prompt 框。
...
可以通过配置 `KiyoMessageBox` 的参数来实现一些个性化需求（options 参数见下文）。
...
`KiyoMessageBox` 提供了全局方法 `KiyoMessageBox.alert`、`KiyoMessageBox.confirm`、`KiyoMessageBox.prompt`，用于在需要时弹出消息弹框。
如果完整引入了 `KiyoicUI`, 则会为 `app.config.globalProperties` 添加全局方法 `$msgbox`、`$alert`、`$confirm`、`$prompt`。
