...
describe("Popconfirm/index.ts", () => {
  ...
  it("should be exported with withInstall()", () => {
    expect(KiyoPopconfirm.install).toBeDefined();
  });

  // 测试 Popconfirm 组件是否被正确导出
  it("should be exported Popconfirm component", () => {
    expect(KiyoPopconfirm).toBe(Popconfirm);
  });
  ...
  test("should enhance Popconfirm component", () => {
    ...
    expect(enhancedPopconfirm).toBe(KiyoPopconfirm);
...
describe("Popconfirm.vue", () => {
  ...
  test("popconfirm emits", async () => {
    ...
    // 弹出层是否出现
    expect(wrapper.find(".kiyo-popconfirm").exists()).toBeTruthy();
    const confirmButton = wrapper.find(".kiyo-popconfirm__confirm");
    expect(confirmButton.exists()).toBeTruthy();

    confirmButton.trigger("click");
    await vi.runAllTimers();
    expect(wrapper.find(".kiyo-popconfirm").exists()).toBeFalsy();
    expect(onConfirm).toBeCalled();

    triggerArea.trigger("click");
    await vi.runAllTimers();
    expect(wrapper.find(".kiyo-popconfirm").exists()).toBeTruthy();
    const cancelButton = wrapper.find(".kiyo-popconfirm__cancel");
