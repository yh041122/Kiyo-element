...
import { KiyoDropdown, KiyoDropdownItem } from ".";
...
describe("Dropdown/index.ts", () => {
  ...
  it("should be exported with withInstall()", () => {
    expect(KiyoDropdown.install).toBeDefined();
    expect(KiyoDropdownItem.install).toBeDefined();
  });

  // 测试 Dropdown 组件是否被正确导出
  it("should be exported Dropdown component", () => {
    expect(KiyoDropdown).toBe(Dropdown);
    expect(KiyoDropdownItem).toBe(DropdownItem);
  });

  // 可选：测试 withInstall 是否增强了 Tooltip 组件的功能
  test("should enhance Dropdown component", () => {
    const enhancedDropdown = withInstall(Dropdown);
    expect(enhancedDropdown).toBe(KiyoDropdown);
...
describe("Dropdown.vue", () => {
  ...
  it("should render slots correctly", () => {
    ...
    expect(wrapper.find(".kiyo-dropdown").exists()).toBeTruthy();
  });
  // 测试 DropdownItem 组件是否触发 command 事件
  it("should emit command event when item is clicked", async () => {
    ...
    expect(wrapper.find(".kiyo-dropdown__menu").exists()).toBeTruthy();
  ...
  it("should toggle visibility when split btn is clicked", async () => {
    ...
    expect(wrapper.find(".kiyo-dropdown__menu").exists()).toBeFalsy();
